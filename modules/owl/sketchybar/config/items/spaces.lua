...
local function update_workspace()
  sbar.exec("aerospace list-windows --format '%{workspace}|%{app-name}' --all", function(windows_output)
    ...
    for line in windows_output:gmatch("[^\r\n]+") do
      local workspace, app = line:match(workspace_app_pattern)
...
local workspace_names = parse_string_to_table(result)
...
for i, space_name in ipairs(workspace_names) do 
  ...
  space_item:subscribe("aerospace_workspace_change", function(env)
    local is_active = env.FOCUSED_WORKSPACE == space_name
    spaces_props[space_name].is_active = is_active

    local query_result = space_item:query()
    local is_hovered = false
    if query_result and query_result.mouse then is_hovered = query_result.mouse.over == "true" end
    ...
    sbar.animate("tanh", 30, function()
      space_item:set({
        ...
        background = {
          border_color = is_active and colors.red and colors.bg2,
          color        = is_active and colors.with_alpha(colors.red, 0.2) or colors.bg1
        }
      })
      bracket_item:set({
        background = {
          border_color = is_active and colors.red = colors.bg2,
